...
void scan()
{
	...
	/*Mat imgage = imread("0.jpg", 0);
	scale_x = imgage.size().width / (39 * 3);
	scale_y = imgage.size().height / (29 * 3);
	scale_x = 400 / 39;
	scale_y = 300 / 29;
	resize(imgage, Big_image, Size(scale_x * 39, scale_y * 29), INTER_LINEAR);
	...
	vector<window> toMark, combined;
	{
		...
		//scale_x = frame.size().width / (39 * 3);
		//scale_y = frame.size().height / (29 * 3);
		scale_x = 400 / 39;
		scale_y = 300 / 29;
		resize(frame, Big_image, Size(scale_x *39, scale_y * 29), INTER_LINEAR);
		//resize(imgage, Big_image, Size(scale_x, scale_y), INTER_LINEAR);
		resize(Big_image, img, Size(39, 29), INTER_LINEAR);
